/**
 * POST /api/findings/:id/dismiss — dismiss a finding with a reason.
 *
 * No-mutation contract: only changes finding state, never mutates ledger.
 * Reads finding ID from URL param, expectedVersion and reason from JSON body.
 *
 * Request body: { expectedVersion: number, reason: string }
...
export default defineEventHandler(async (event) => {
  ...
  if (!reason) {
    setResponseStatus(event, 400);
    return errorEnvelope('MISSING_REASON', 'Dismissal reason is required.', authInfo, false, requestId);
  }
  ...
  try {
    const finding = await wf.store.dismissFinding({
      findingId,
    ...
    const safe = sanitizeError(error, requestId, 'DISMISS_FAILED', false);
