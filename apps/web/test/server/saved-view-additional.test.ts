...
const SAMPLE_VIEW = { viewId: 'view_001', name: 'My View', viewType: 'attention', scope: {}, sort: null, lastUsedAt: null, createdAt: '2026-07-27T10:00:00Z' };
...
describe('POST /api/reports/views/[id]/duplicate', () => {
  ...
  it('must duplicate a saved view', async () => {
    ...
    mockStore.duplicateSavedView.mockResolvedValue({ ...SAMPLE_VIEW, viewId: 'view_002', name: 'Duplicated View' });
...
describe('PATCH /api/reports/views/[id]/last-used', () => {
  ...
  it('must record last used timestamp', async () => {
    mockGetRouterParam.mockReturnValue('view_001');
    mockStore.getSavedView.mockResolvedValue(SAMPLE_VIEW);
    mockStore.recordSavedViewUsage.mockResolvedValue({ ...SAMPLE_VIEW, lastUsedAt: '2026-07-27T12:00:00Z' });
