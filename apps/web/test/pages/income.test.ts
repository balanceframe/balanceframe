...
const sampleResult = {
  sources: [
    { name: 'Salary', typicalMonthly: { minorUnits: '500000', currency: 'USD' }, reliabilityScore: 95, variability: 2, paymentCount: 24, isRegular: true },
    { name: 'Freelance', typicalMonthly: { minorUnits: '120000', currency: 'USD' }, reliabilityScore: 60, variability: 35, paymentCount: 12, isRegular: false },
  ],
...
describe('Income page', () => {
  ...
  it('shows empty state when no sources', async () => {
    mockFetch.mockResolvedValue(okEnvelope({ sources: [], totalMonthly: null, overallScore: null, unreliableSourceCount: 0 }));
    const wrapper = shallowMount(IncomePage, { global: { stubs } });
    await flushPromises();
