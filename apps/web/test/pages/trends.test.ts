...
const sampleResult = {
  categoryVariances: [
    { categoryId: 'cat-1', categoryName: 'Groceries', budgeted: { minorUnits: '50000', currency: 'USD' }, actual: { minorUnits: '55000', currency: 'USD' }, variance: { minorUnits: '5000', currency: 'USD' }, variancePercent: 10, label: 'overspent' },
    { categoryId: 'cat-2', categoryName: 'Utilities', budgeted: { minorUnits: '20000', currency: 'USD' }, actual: { minorUnits: '18000', currency: 'USD' }, variance: { minorUnits: '-2000', currency: 'USD' }, variancePercent: -10, label: 'on_track' },
  ],
  trends: [
    { categoryId: 'cat-1', categoryName: 'Groceries', direction: 'increasing', avgChange: 5.2, periodsAnalyzed: 6, seasonalityDetected: false },
    { categoryId: 'cat-2', categoryName: 'Utilities', direction: 'stable', avgChange: 0.3, periodsAnalyzed: 6, seasonalityDetected: true },
  ],
...
describe('Trends page', () => {
  ...
  it('calls /api/trends-variance on mount', async () => {
    ...
    await flushPromises();
    expect(mockFetch).toHaveBeenCalledWith('/api/trends-variance', expect.objectContaining({ query: expect.any(Object) }));
  });
  ...
  it('shows empty state when no variances', async () => {
    mockFetch.mockResolvedValue(okEnvelope({ categoryVariances: [], trends: [], totalBudgeted: null, totalActual: null, totalVariance: null, overallVariancePercent: null }));
    const wrapper = shallowMount(TrendsPage, { global: { stubs } });
    await flushPromises();
