...
vi.mock('../../lib/auth-client', () => ({
  authClient: {
    useSession: () => ({ value: { data: { user: { email: 'a11y@example.test' } } } }),
    signOut: vi.fn(),
  },
}));
...
const layoutStubs = {
  ...
  FreshnessBanner: {
    template: '<span aria-label="Data freshness">Current</span>',
    props: ['freshness', 'showRefresh'],
  },
};
...
beforeEach(() => {
  ...
  fetchMock.mockResolvedValue(
    envelope({ overallScore: null, dimensions: [], recommendations: [] }),
  );
});
...
describe('Phase 8.5 accessibility acceptance', () => {
  it('keeps keyboard focus indicators on navigation and controls', async () => {
    ...
    expect(
      links.every((link) => link.classes().some((name) => name.includes('focus-visible'))),
    ).toBe(true);
    ...
    const wrapper = shallowMount(DataQualityPage, {
      global: {
        stubs: {
          AnalysisPage: {
            template:
              '<section><div v-if="error" role="alert">Error</div><slot name="content"/></section>',
            props: ['error'],
          },
          ...
          AnalysisTable: {
            template:
              '<table aria-label="Analysis results"><caption>Data quality</caption><tbody><tr><td>Unknown</td></tr></tbody></table>',
            props: ['columns', 'rows'],
          },
        },
      },
