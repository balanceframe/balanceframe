...
vi.mock('../../lib/auth-client', () => ({
  authClient: { useSession: () => ({ value: { data: { user: { email: 'owner@example.com' } } } }), signOut },
}));

const stubs = {
  UContainer: { template: '<div><slot /></div>' },
  UButton: { template: '<button v-bind="$attrs" @click="$emit(\'click\')"><slot /></button>', emits: ['click'] },
  NuxtLink: { props: ['to'], template: '<a :href="to" @click.prevent="$emit(\'click\')"><slot /></a>', emits: ['click'] },
};
...
describe('shared shell scope', () => {
  it('shows current space, authorization scope, sync status and user menu sign out', async () => {
    vi.stubGlobal('$fetch', vi.fn().mockResolvedValue({
      status: 'ok',
      result: { month: '2026-08', budgetName: 'Household', totalRemaining: { minorUnits: '100', currency: 'USD' } },
  ...
  it('keeps mobile navigation keyboard accessible and reports pending state', async () => {
    vi.stubGlobal('$fetch', vi.fn().mockResolvedValue({ status: 'error', result: null, dataFreshness: null, authorization: null }));
    const wrapper = mount(DefaultLayout, { global: { stubs }, slots: { default: '<p>content</p>' } });
    const toggle = wrapper.find('button[aria-label="Toggle navigation menu"]');
    expect(toggle.attributes('aria-expanded')).toBe('false');
    await toggle.trigger('click');
    expect(toggle.attributes('aria-expanded')).toBe('true');
    expect(wrapper.find('nav[aria-label="Mobile navigation"] a').attributes('class')).toContain('focus-visible');
...
describe('FreshnessBanner', () => {
  it('uses a real focusable refresh control', () => {
    const wrapper = mount(FreshnessBanner, { props: { freshness: { isStale: true, lastSync: null, label: 'stale' }, showRefresh: true } });
