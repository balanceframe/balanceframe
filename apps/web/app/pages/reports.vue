      ...
      <div class="space-y-6">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
          <UButton
            ...
            :key="rt.value"
            :variant="reportType === rt.value ? 'solid' : 'outline'"
            size="sm"
        ...
        <div v-if="reportResult" class="mt-4">
          <UCard>
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400" data-testid="report-id">
              Report ID: {{ reportResult.reportId }}
            </p>
            <p v-if="reportResult.label" class="text-sm mt-1 text-gray-600 dark:text-gray-400">
              Label: {{ reportResult.label }}
            </p>
            <p
              v-if="reportResult.transactionCount"
              class="text-sm mt-1 text-gray-600 dark:text-gray-400"
            >
              Transactions: {{ reportResult.transactionCount }}
            </p>
            <SemanticAmount
              v-if="reportResult.totalAmount"
              :amount="reportResult.totalAmount"
              class="mt-1"
            />
            <p
              v-if="reportResult.generatedAt"
              class="text-xs text-gray-400 dark:text-gray-500 mt-2"
            >
              Generated: {{ reportResult.generatedAt }}
            </p>
            <div
              v-if="reportResult.tags && reportResult.tags.length"
              class="mt-2 flex flex-wrap gap-1"
            >
              <span
                v-for="tag in reportResult.tags"
                :key="tag"
...
const hasData = computed(
  () =>
    historyEntries.value.length > 0 || savedViews.value.length > 0 || reportResult.value !== null,
);
...
function selectReport(val: string) {
...
function createView() {
  ...
  if (!name) return;
  void mutateView('', 'POST', '/api/reports/views', {
    name,
    viewType: 'reports',
    scope: { reportType: reportType.value, monthRange: monthRange.value },
  });
}
...
function updateView(viewId: string) {
  void mutateView(viewId, 'PATCH', undefined, {
    scope: { reportType: reportType.value, monthRange: monthRange.value },
  });
}
  ...
  try {
    ...
    if (res.status === 'ok' && res.result) savedViews.value = res.result.views;
      ...
      viewsError.value = {
        ...
        message: res.error?.message ?? 'Failed to load saved views.',
...
async function generate() {
  ...
  try {
    ...
    if (res.status === 'ok' && res.result) reportResult.value = res.result;
      ...
      error.value = {
        code: res.error?.code ?? 'GENERATE_FAILED',
        message: res.error?.message ?? 'Report generation returned an error.',
      };
  } catch (e) {
...
onMounted(async () => {
  try {
    const historyRes =
      await $fetch<Envelope<{ entries: ReportHistoryEntry[]; total: number }>>(
        '/api/reports/history',
      );
    if (historyRes.status === 'ok' && historyRes.result) {
      ...
      error.value = {
        code: historyRes.error?.code ?? 'HISTORY_FAILED',
        message: historyRes.error?.message ?? 'Failed to load report history.',
      };
    }
    await loadViews();
  } catch (e) {
