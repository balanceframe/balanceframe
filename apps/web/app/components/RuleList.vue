  ...
  <UCard>
    <template #header>
      <div class="flex items-center justify-between">
        ...
        <UBadge v-if="loading" color="warning" variant="soft" label="Loading..." />
        <UBadge v-else color="neutral" variant="solid" size="sm" :label="`${items.length} rules`" />
      </div>
    </template>

    <div v-if="items.length === 0 && !loading" class="text-center py-8">
      <p class="text-gray-500 dark:text-gray-400">No rules configured.</p>
    </div>
