...
|---|---|---|
| Uncategorized | 40% | `category_id` is `null` |
| Pending | 20% | `imported_payee` is set AND `cleared` is `"uncleared"` |
| Cleared | 30% | `cleared` is `"cleared"` or `"reconciled"` |
| Both (uncategorized + pending) | 10% | `category_id` is `null` AND `imported_payee` is set AND `cleared` is `"uncleared"` |
...
- Synced accounts carry transactions with `imported_payee` set for the majority of those transactions; manual accounts rarely set `imported_payee`.
...
Certain accounts carry non-standard `transfer_from` / `transfer_to` values that override the default transfer account mapping:

| Account | Override |
|---|---|
| `a_3` (Credit Card) | `transfer_from` set to `"a_1"` (checking) — credit-card payments pull from checking |
| `a_1` (Checking) | `transfer_to` set to `"a_6"` (loan) — loan payments originate from checking |
...
- Every account, category, payee, and transaction is included in the snapshot **unless** it carries `is_excluded: true`.
- Excluded accounts appear in `accounts[]` with `is_excluded: true` but their transactions are omitted from the snapshot.
...
Example pair:
- `payee: "AMAZON MKTPLACE"` on 2026-07-10 for -2399
- `payee: "Amazon Marketplace"` on 2026-07-11 for -2399
...
At least one account in the fixture has an unreconciled balance that has not changed for **90+ days**:

- Account `a_stale` has its last `reconciled_date` set to `2026-04-01` (105 days before the reference date).
- Its balance is unchanged between that date and the snapshot date.
- The stale-account check must flag this account's balance as unreliable for envelope-availability calculations.

## 8. Deleted / Renamed Categories and Version-Changed Records

### Deleted categories
- A deleted category appears in `categories[]` with `deleted: true` and `version >= 2`.
- At least one transaction references this deleted category via `category_id`.
- The fixture consumer must detect the reference to a deleted category and surface it as a data-quality concern.

### Renamed categories
- A renamed category appears with its current `name` and an `old_name` metadata field recording its previous name.
