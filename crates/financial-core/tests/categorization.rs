...
fn test_inference_policy_camel_case_serde() {
    ...
    assert_eq!(
        json, "\"disabled\"",
        "Disabled must serialize as 'disabled'"
    );

    let json = serde_json::to_string(&InferencePolicy::LocalOnly).unwrap();
    assert_eq!(
        json, "\"localOnly\"",
        "LocalOnly must serialize as 'localOnly'"
    );

    let json = serde_json::to_string(&InferencePolicy::ExternalAllowed).unwrap();
    assert_eq!(
        json, "\"externalAllowed\"",
        "ExternalAllowed must serialize as 'externalAllowed'"
    );
...
fn test_provenance_round_trip() {
    ...
    assert!(
        json.contains("payloadHash"),
        "payloadHash must be camelCase"
    );
    assert!(
        json.contains("inferencePolicyVersion"),
        "inferencePolicyVersion must be camelCase"
    );
}
...
fn test_eligibility_ranks_evidence_explicitly_not_position() {
    ...
    let candidate = sample_candidate(
        ...
        vec![
            ...
            Evidence::new(
                EvidenceKind::Historical,
                "Categorized as Food on 2026-07-01",
            ),
