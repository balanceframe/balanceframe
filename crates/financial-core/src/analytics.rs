...
pub fn compute_data_quality_center(
    ...
    if accounts_count == 0 && transactions_count == 0 {
        return DataQualityCenter {
            ...
            overall_score: None,
            dimensions: vec![],
            recommendations: vec!["Add accounts and transactions to enable quality assessment.".to_string()],
        };
    }
    ...
    let (completeness_score, completeness_explanation) = if transactions_count == 0 {
        ...
        let explanation = if uncategorized_ratio > 0.3 {
            format!("{:.0}% of transactions are uncategorized.", uncategorized_ratio * 100.0)
        } else if uncategorized_ratio > 0.1 {
            format!("{:.0}% uncategorized — manageable.", uncategorized_ratio * 100.0)
        } else {
    ...
    dimensions.push(QualityDimension {
        ...
        worst_severity: if uncategorized_count > 0 { Some("warning".to_string()) } else { None },
    });
    ...
    let (freshness_score, freshness_explanation) = match (stale_account_days, stale_bank_sync_days) {
        ...
        (Some(days), None) => {
            ...
            (score, format!("Account data is {} days old; sync unknown.", days))
        }
        (None, Some(days)) => {
            let score = (1.0_f64 - (days as f64 / 90.0_f64)).clamp(0.0, 1.0);
            (score, format!("Bank sync is {} days old; account age unknown.", days))
        }
    ...
    dimensions.push(QualityDimension {
        ...
        worst_severity: if freshness_score < 0.5 { Some("stale".to_string()) } else { None },
    });
    ...
    let (consistency_score, consistency_explanation) = if duplicate_candidates > 0 {
        let score = (1.0_f64 - (duplicate_candidates as f64 / transactions_count.max(1) as f64)).clamp(0.0, 1.0);
        (score, format!("{} potential duplicate pairs found.", duplicate_candidates))
    } else {
    ...
    dimensions.push(QualityDimension {
        ...
        worst_severity: if duplicate_candidates > 0 { Some("warning".to_string()) } else { None },
    });
    ...
    if uncategorized_count > 0 {
        recommendations.push(format!("Categorize {} uncategorized transactions.", uncategorized_count));
    }
    if duplicate_candidates > 0 {
        recommendations.push(format!("Review {} potential duplicate transactions.", duplicate_candidates));
    }
...
pub fn compute_liquidity_coverage(
    ...
    let currency = total_liquid.as_ref().map(|m| m.currency().to_string()).unwrap_or_default();
    ...
    for bm in budget_months {
        if bm.month >= current_month.to_string() {
            for (_cat_id, bc) in &bm.categories {
                if bc.amount.minor_units() > 0 {
                    total_obligation_minor = total_obligation_minor.saturating_add(bc.amount.minor_units());
                }
            }
        }
    }
    ...
    if let Some(ref liquid) = total_liquid {
        ...
        if total_obligation_minor > 0 {
            let ratio_30 = if total_obligation_minor > 0 {
                ...
                let current_obligations: i64 = obligations.iter()
...
pub fn compute_budget_variance(
    ...
    for (cat_id, bc) in &latest.categories {
        ...
        let actual_minor: i64 = transactions.iter()
        ...
        let label = if variance_minor < 0 { "over" } else if variance_minor > 0 { "under" } else { "on_track" };
    ...
    if budget_months.len() >= 2 {
        ...
        let cat_names: std::collections::HashMap<String, String> = categories.iter()
            .map(|c| (c.id.clone(), c.name.clone()))
            .collect();

        // For each category, compute average change across months
        let mut cat_spending: std::collections::HashMap<String, Vec<(String, i64)>> = std::collections::HashMap::new();
        for bm in budget_months {
            for (cat_id, bc) in &bm.categories {
                cat_spending.entry(cat_id.clone())
                    .or_default()
        ...
        for (cat_id, amounts) in &cat_spending {
            ...
            let seasonality = detect_seasonality(&amounts.iter().map(|(_, v)| *v).collect::<Vec<_>>());
            trends.push(CategoryTrend {
                category_id: cat_id.clone(),
                category_name: cat_names.get(cat_id).cloned().unwrap_or_else(|| cat_id.clone()),
                direction,
                avg_change,
    ...
    let overall_variance_percent = if total_budgeted_minor > 0 {
        Some((total_budgeted_minor - total_actual_minor) as f64 / total_budgeted_minor as f64 * 100.0)
    } else {
        None
    };
    ...
    BudgetVarianceReport {
        ...
        trends,
        total_budgeted: currency.as_ref().map(|cur| Money::new(total_budgeted_minor, cur)),
        total_actual: currency.as_ref().map(|cur| Money::new(total_actual_minor, cur)),
        total_variance: currency.as_ref().map(|cur| Money::new((total_budgeted_minor - total_actual_minor).unsigned_abs() as i64, cur)),
...
fn classify_irregularity(frequency: &str, _amount: i64, all_schedules: &[crate::snapshots::Schedule]) -> IrregularityKind {
    match frequency {
        "monthly" | "everyMonth" | "semimonthly" | "biweekly" => IrregularityKind::NonMonthly,
        "quarterly" | "everyQuarter" | "yearly" | "everyYear" | "semiannually" => IrregularityKind::Seasonal,
        "once" | "oneTime" => IrregularityKind::OneOff,
        _ => {
            // Variable amount: check if other schedules with same name have different amounts
            let amounts: Vec<i64> = all_schedules.iter()
...
pub fn compute_income_reliability(
    ...
    let income_cat_ids: std::collections::HashSet<String> = categories.iter()
    ...
    let mut income_by_payee: std::collections::HashMap<String, Vec<i64>> = std::collections::HashMap::new();
    let mut currency: Option<String> = None;
    ...
    for tx in transactions {
        ...
        let in_income_cat = tx.category_id.as_ref().map(|cid| income_cat_ids.contains(cid)).unwrap_or(false);
        if !in_income_cat {
            continue;
        }
        let payee = tx.payee_name.clone().unwrap_or_else(|| tx.imported_payee.clone().unwrap_or_else(|| "unknown".to_string()));
        income_by_payee.entry(payee).or_default().push(tx.amount.minor_units());
    ...
    for sched in schedules {
        if sched.amount.minor_units() > 0 {
            income_by_payee.entry(sched.payee_name.clone().unwrap_or_else(|| sched.id.clone())).or_default().push(sched.amount.minor_units());
            currency = Some(sched.amount.currency().to_string());
        }
    }
    ...
    for (name, amounts) in &income_by_payee {
        ...
        let variance = if count > 1 {
            amounts.iter().map(|a| (*a as f64 - mean).powi(2)).sum::<f64>() / (count - 1) as f64
        } else {
            0.0
        };
        ...
        sources.push(IncomeSource {
            name: name.clone(),
            typical_monthly: currency.as_ref().map(|cur| Money::new(typical, cur)).unwrap_or_else(|| Money::new(typical, "USD")),
    ...
    sources.sort_by(|a, b| b.typical_monthly.minor_units().cmp(&a.typical_monthly.minor_units()));

    IncomeReliabilityReport {
        availability: AnalysisAvailability::Available,
        overall_score: Some(if !sources.is_empty() { score_sum / sources.len() as f64 } else { 0.0 }),
...
pub fn compute_forecast_calibration(
    ...
    if budget_months.len() < 2 {
        return ForecastCalibration {
            availability: if budget_months.is_empty() { AnalysisAvailability::NoConfiguration } else { AnalysisAvailability::InsufficientData },
            metrics: vec![],
            overall_calibrated: false,
            recommendations: vec!["At least 2 budget months are needed for calibration.".to_string()],
        };
    }
    ...
    for bm in budget_months {
        let month_forecast: i64 = bm.categories.values()
            .map(|bc| if bc.amount.minor_units() > 0 { bc.amount.minor_units() } else { 0 })
            .sum();
        income_forecast.push((bm.month.clone(), month_forecast));

        let month_actual: i64 = transactions.iter()
    ...
    if let Some(ref m) = income_metric {
        if !m.is_calibrated {
            ...
            recommendations.push("Income forecasts show systematic bias — review income schedules.".to_string());
        }
    }
    ...
    for bm in budget_months {
        let month_forecast: i64 = bm.categories.values()
            .map(|bc| if bc.amount.minor_units() < 0 { bc.amount.minor_units().unsigned_abs() as i64 } else { 0 })
            .sum();
    ...
    if let Some(ref m) = expense_metric {
        if !m.is_calibrated {
            all_calibrated = false;
            recommendations.push("Expense forecasts show systematic bias — review budget categories.".to_string());
        }
    }
...
pub fn compare_scenarios(
    ...
    if let (Some(base_obj), Some(comp_obj)) = (baseline.payload.as_object(), comparison.payload.as_object()) {
        let all_keys: std::collections::BTreeSet<String> = base_obj.keys()
            .chain(comp_obj.keys())
            .cloned()
            .collect();
...
pub fn compute_multidimensional_health(
    ...
    if let Some(lc) = liquidity_coverage {
        if lc.availability == AnalysisAvailability::Available {
            let score = lc.coverage.first().map(|c| {
                if c.ratio >= f64::MAX { 1.0 }
                else if c.ratio >= 3.0 { 0.9 }
                else if c.ratio >= 2.0 { 0.8 }
                else if c.ratio >= 1.5 { 0.7 }
                else if c.ratio >= 1.0 { 0.6 }
                else if c.ratio >= 0.5 { 0.4 }
                else { 0.2 }
            }).unwrap_or(0.5);
            let severity = if score >= 0.7 { "good".to_string() } else if score >= 0.4 { "fair".to_string() } else { "critical".to_string() };
            dimensions.push(HealthDimension {
                dimension: "liquidity".to_string(),
                score,
                weight: 0.25,
                explanation: format!("Coverage ratio: {}", lc.coverage.first().map(|c| format!("{:.2}", c.ratio)).unwrap_or_else(|| "N/A".to_string())),
                severity,
            });
            weight_sum += 0.25;
            weighted_score += score * 0.25;
            if score < 0.5 {
                recommendations.push("Build liquid reserves to improve obligation coverage.".to_string());
            }
        }
    }
    ...
    if let Some(bv) = budget_variance {
        if bv.availability == AnalysisAvailability::Available {
            ...
            let score = if variance_pct >= 10.0 { 0.9 }
                else if variance_pct >= 5.0 { 0.8 }
                else if variance_pct >= 0.0 { 0.7 }
                else if variance_pct >= -10.0 { 0.5 }
                else { 0.3 };
            let severity = if score >= 0.7 { "good".to_string() } else if score >= 0.4 { "fair".to_string() } else { "critical".to_string() };
            dimensions.push(HealthDimension {
    ...
    if let Some(ir) = income_reliability {
        if ir.availability == AnalysisAvailability::Available {
            ...
            if score < 0.5 {
                recommendations.push("Income sources show volatility — consider building larger emergency fund.".to_string());
            }
        }
    }
    ...
    if let Some(fc) = forecast_calibration {
        if fc.availability == AnalysisAvailability::Available {
            ...
            let severity = if score >= 0.7 { "good".to_string() } else { "fair".to_string() };
            dimensions.push(HealthDimension {
                dimension: "forecast_accuracy".to_string(),
                score,
                weight: 0.15,
                explanation: if fc.overall_calibrated { "Forecasts are well-calibrated.".to_string() } else { "Forecasts show systematic bias.".to_string() },
                severity,
            });
            weight_sum += 0.15;
            weighted_score += score * 0.15;
            if !fc.overall_calibrated {
                recommendations.push("Improve forecast accuracy by reviewing budget assumptions.".to_string());
            }
        }
    }
    ...
    if let Some(dq) = data_quality {
        if dq.availability == AnalysisAvailability::Available {
            ...
            weighted_score += score * 0.15;
            if score < 0.5 {
                recommendations.push("Improve data quality for more accurate health assessment.".to_string());
            }
        }
    }
    ...
    if dimensions.is_empty() {
        return MultidimensionalHealth {
            ...
            recommendations: vec!["Configure budgets, accounts, and categories to enable health assessment.".to_string()],
        };
    }

    let composite = if weight_sum > 0.0 { weighted_score / weight_sum } else { 0.0 };
...
mod tests {
    ...
    fn make_budget_month(month: &str, categories: Vec<(&str, i64)>) -> BudgetMonth {
        BudgetMonth {
            ...
            categories: categories.into_iter().map(|(id, amount)| {
                (id.to_string(), BudgetCategory {
    ...
    fn test_data_quality_center_stale_freshness() {
        ...
        assert!(result.overall_score.unwrap() < 0.5);
        assert!(result.recommendations.iter().any(|r| r.contains("Reconnect")));
    }
    ...
    fn test_data_quality_center_duplicates_detected() {
        ...
        assert!(result.recommendations.iter().any(|r| r.contains("duplicate")));
    }
    ...
    fn test_liquidity_coverage_with_budget_data() {
        ...
        let liquid = make_money(5000, "USD");
        let result = compute_liquidity_coverage(Some(&liquid), &[], &[make_budget_month("2026-07", vec![("c1", 100)])], "2026-07");
        assert_eq!(result.availability, AnalysisAvailability::Available);
    ...
    fn test_budget_variance_under_budget() {
        ...
        let transactions = vec![make_transaction("tx1", -200, Some("c1"), Some("Grocery"), "2026-07-15")];
    ...
    fn test_budget_variance_over_budget() {
        let categories = vec![make_category("c1", "Food", false)];
        let budgets = vec![make_budget_month("2026-07", vec![("c1", 500)])];
        let transactions = vec![make_transaction("tx1", -600, Some("c1"), Some("Grocery"), "2026-07-15")];
        let result = compute_budget_variance(&budgets, &transactions, &categories, "2026-07-27");
    ...
    fn test_budget_variance_on_track() {
        let categories = vec![make_category("c1", "Food", false)];
        let budgets = vec![make_budget_month("2026-07", vec![("c1", 500)])];
        let transactions = vec![make_transaction("tx1", -500, Some("c1"), Some("Grocery"), "2026-07-15")];
        let result = compute_budget_variance(&budgets, &transactions, &categories, "2026-07-27");
    ...
    fn test_irregular_obligations_quarterly() {
        let schedules = vec![
            make_schedule("Insurance", -600, "quarterly", "2026-07-01", Some("c1")),
        ];
    ...
    fn test_irregular_obligations_one_off() {
        let schedules = vec![
            make_schedule("Birthday Gift", -100, "once", "2026-08-15", Some("c1")),
        ];
        let result = compute_irregular_obligations(&schedules);
        assert_eq!(result.availability, AnalysisAvailability::Available);
    ...
    fn test_irregular_obligations_annual_estimate() {
        let schedules = vec![
            make_schedule("Quarterly Tax", -1200, "quarterly", "2026-07-01", Some("c1")),
        ];
        let result = compute_irregular_obligations(&schedules);
    ...
    fn test_income_reliability_regular_income() {
        ...
        let transactions = vec![
            make_transaction("tx1", 5000, Some("c_income"), Some("Employer"), "2026-05-01"),
            make_transaction("tx2", 5000, Some("c_income"), Some("Employer"), "2026-06-01"),
            make_transaction("tx3", 5000, Some("c_income"), Some("Employer"), "2026-07-01"),
        ];
    ...
    fn test_income_reliability_insufficient_data() {
        let categories = vec![make_category("c_income", "Salary", true)];
        let transactions = vec![
            make_transaction("tx1", 100, Some("c_income"), Some("Gig"), "2026-07-01"),
        ];
        let result = compute_income_reliability(&transactions, &[], &categories);
        assert_eq!(result.availability, AnalysisAvailability::Available);
    ...
    fn test_income_reliability_variable_income() {
        ...
        let transactions = vec![
            make_transaction("tx1", 3000, Some("c_income"), Some("Client A"), "2026-05-01"),
            make_transaction("tx2", 1000, Some("c_income"), Some("Client A"), "2026-06-01"),
            make_transaction("tx3", 5000, Some("c_income"), Some("Client A"), "2026-07-01"),
        ];
        let result = compute_income_reliability(&transactions, &[], &categories);
        assert_eq!(result.availability, AnalysisAvailability::Available);
    ...
    fn test_income_reliability_schedules_income() {
        let categories = vec![make_category("c_income", "Salary", true)];
        let schedules = vec![
            make_schedule("Employer", 5000, "monthly", "2026-07-01", Some("c_income")),
        ];
    ...
    fn test_scenario_comparison_identical() {
        ...
        let baseline = Scenario {
            id: ScenarioId { id: "sc1".to_string(), name: "Baseline".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
        ...
        let comparison = Scenario {
            id: ScenarioId { id: "sc2".to_string(), name: "Comparison".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
            assumptions: vec![],
            expires_at: "2026-12-31".to_string(),
            payload,
    ...
    fn test_scenario_comparison_unavailable_id() {
        let empty = Scenario {
            id: ScenarioId { id: "".to_string(), name: "Empty".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
            assumptions: vec![],
            expires_at: "2026-12-31".to_string(),
    ...
    fn test_scenario_comparison_insufficient_data() {
        let scenario = Scenario {
            id: ScenarioId { id: "sc1".to_string(), name: "No Payload".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
            assumptions: vec![],
            expires_at: "2026-12-31".to_string(),
    ...
    fn test_multidimensional_health_with_data() {
        let liquid = make_money(10000, "USD");
        let schedules = vec![make_schedule("Rent", -2000, "monthly", "2026-07-01", Some("c1"))];
        ...
        let result = compute_multidimensional_health(Some(&liquidity), Some(&variance), None, None, None);
        assert_eq!(result.availability, AnalysisAvailability::Available);
        assert!(!result.dimensions.is_empty());
    ...
    fn test_multidimensional_health_composite_score() {
        ...
        let result = compute_multidimensional_health(Some(&liquidity), Some(&variance), None, None, Some(&dq));
        assert!(result.composite_score > 0.0 && result.composite_score <= 1.0);
        assert!(!result.recommendations.is_empty() || result.summary.len() > 0);
    }
    ...
    fn test_multidimensional_health_recommendations() {
        ...
        let categories = vec![make_category("c1", "Food", false)];
        let budgets = vec![make_budget_month("2026-07", vec![("c1", 500)])];
        let transactions = vec![make_transaction("tx1", -800, Some("c1"), Some("Grocery"), "2026-07-15")];
    ...
    fn test_scenario_id_serialization_roundtrip() {
        let sid = ScenarioId { id: "test-1".to_string(), name: "Test Scenario".to_string() };
    ...
    fn test_scenario_stable_id_immutability() {
        let s1 = Scenario {
            id: ScenarioId { id: "sc-immutable-1".to_string(), name: "Immutable".to_string() },
            version: ScenarioVersion { source_version: "1.0".to_string(), result_version: "1.0".to_string() },
    ...
    fn test_compute_budget_variance_handles_zero_budget() {
        ...
        let transactions = vec![make_transaction("tx1", -100, Some("c1"), Some("Store"), "2026-07-15")];
        let result = compute_budget_variance(&budgets, &transactions, &categories, "2026-07-27");
        assert_eq!(result.category_variances[0].label, "over");
    ...
    fn test_compute_forecast_calibration_zero_forecast() {
        ...
        let transactions = vec![
            make_transaction("tx1", -100, Some("c1"), Some("Store"), "2026-06-15"),
        ];
        let result = compute_forecast_calibration(&budgets, &transactions);
        assert_eq!(result.availability, AnalysisAvailability::Available);
    }
    ...
    fn test_budget_variance_projected_behind_spent_separate() {
        ...
        let budgets = vec![make_budget_month("2026-07", vec![("c1", 500)])];
        let transactions = vec![
            make_transaction("tx1", -200, Some("c1"), Some("Grocery"), "2026-07-15"),
        ];
        let result = compute_budget_variance(&budgets, &transactions, &categories, "2026-07-27");
    ...
    fn test_budget_variance_currency_label_preserved() {
        ...
        let budgets = vec![make_budget_month("2026-07", vec![("c1", 500)])];
        let transactions = vec![
            make_transaction("tx1", -100, Some("c1"), Some("Store"), "2026-07-15"),
        ];
        let result = compute_budget_variance(&budgets, &transactions, &categories, "2026-07-27");
    ...
    fn test_scenario_comparison_preserves_both_ids() {
        let baseline = Scenario {
            id: ScenarioId { id: "base-001".to_string(), name: "Baseline".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
            assumptions: vec![],
            expires_at: "2026-12-31".to_string(),
        ...
        let comparison = Scenario {
            id: ScenarioId { id: "comp-002".to_string(), name: "Comparison".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "2".to_string() },
            assumptions: vec![],
            expires_at: "2026-12-31".to_string(),
    ...
    fn test_scenario_non_mutation_on_compare() {
        ...
        let baseline = Scenario {
            id: ScenarioId { id: "sc-imm-1".to_string(), name: "Immutable Baseline".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
        ...
        let comparison = Scenario {
            id: ScenarioId { id: "sc-imm-2".to_string(), name: "Immutable Comparison".to_string() },
            version: ScenarioVersion { source_version: "1".to_string(), result_version: "1".to_string() },
            assumptions: vec![],
            expires_at: "2026-12-31".to_string(),
            payload: payload2,
